...
class MainPanel extends BaseWidget {
  ...
  update (notify = true) {
    ...
    const userColumns = this.config.visibleFields.filter(vf => !_.some(defColumns, {key: vf})).map(uc => ({title: uc.toUpperCase(), key: uc}));

    const columns = [];
    for (const vf in this.config.visibleFields) {
      for(const dc in defColumns) {
        if(defColumns[dc].key === this.config.visibleFields[vf]) {
          if (defColumns[dc].key === 'message') {
            columns.push(_.find(defColumns, {key: 'data'}));
          }
          columns.push(defColumns[dc]);
        }
      }
      for(const uc in userColumns) {
        if(userColumns[uc].key === this.config.visibleFields[vf] && !_.some(columns, {key: this.config.visibleFields[vf]})) {
          columns.push(userColumns[uc]);
        }
      }
    }
